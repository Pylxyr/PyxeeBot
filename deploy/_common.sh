...
run_setup() {
  ...
  sudo apt update
  sudo apt install -y python3 python3-venv ffmpeg logrotate libopus0 libsodium-dev curl unzip

  echo "[2/11] Installing a JS runtime for yt-dlp (Deno)"
  # yt-dlp needs an external JS runtime to solve YouTube's JS challenges as of
  # the version pinned in requirements.txt — without one, YouTube extraction
  ...
  if command -v deno >/dev/null 2>&1; then
    ...
    # deno_install's install.sh takes zero flags — it's already fully
    # non-interactive (no confirmation prompt to suppress). Its only optional
    # argument is a specific version tag to install instead of latest; passing
    # anything else (a previous version of this script mistakenly passed
    # "--yes") gets treated as that tag, builds a release URL that doesn't
    # exist, and fails every time via `curl --fail`.
  ...
  echo "[4/11] Validating Python runtime"
  ...
  echo "[5/11] Preparing app directories"
  mkdir -p "${APP_DIR}"
  ...
  echo "[9/11] Installing logrotate config"
  ...
  echo "[10/11] Installing systemd unit"
  sed -e "s#/home/ubuntu/musicbot#${APP_DIR}#g" \
